  ...
  return (
    <div className="flex gap-6 mt-15">
      {tasks.map((task, index) => (
        <div
          key={index}
          className={`border rounded-lg shadow-md p-4 w-96 ${task.bgColo}`}
        >
          <div className="flex justify-between items-center">
            <button className="bg-red-500  py-2 px-6 rounded-full mb-4">
              {task.button}
            </button>
            <div className="text-sm mb-4 py-2 px-6 bg-blue-500 rounded-full">
              {task.date}
            </div>
          </div>
